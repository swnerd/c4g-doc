...
<li>Add Database, after adding a database you will be taken to Candidate Selection Wizard.</li>
<li>Upload Gluent Advisor CSV and select one or more tables. On finishing, you will be taken to the Job Wizard.</li>
<li>Name your job, select one or more tables to offload for the job, and schedule the job. At this point, the job will automatically launch based on the schedule.</li>
</ol>
...
It is not manditory to follow the guided flow. For example, if you do not have a Gluent Advisor CSV for Conductor file, you can add tables through the add table wizard on the Candidate Review page. 
...
A site is made up of one or more database, for which jobs can be scheduled for offloading data. At the site level, databases can be added to Conductor, defaults can be set for parameters, notifications or the application, and a site level overview of some metrics are available.
...
<li>Edit the details of a database - Click on the <i>Edit</i> Link of a database to go to the maintain database details page, where you change it's description, icon color, database link information, etc.</li>
</ol>
...
<li>Password     : The password of the CONDUCTOR_FOR_GLUENT user for thie database. [Required field]</li>
</ol>
...
<li>Date the Advisor was run - It is important to select the correct date, as calculations for the defalt offload retention is calculated based on this date and information contained in the Gluent Advisor file.</li>
...
Fields:
<ol>
<li>Search : Enter any search string and press enter or hit Go. Once you have searched, you can clear the critera by hitting the x next to the search term.</li>
</ol>
...
<li>Save : Save any updates to the parition details. The button is located in the Partition Details section.</li>
</ol>
...
Buttons:
<ol>
<li>Maintain Parameters : Pressing the button will bring up the offload parameters page allowing parameters to be added at the table level. See the Command Parameters section for details on adding parameters.</li>
</ol>
...
<ul>
<li>Use Site Default : Use the value specified at the site level in the Application Parameters page.</li>
<li>COUNT    : Before dropping a partition perform a count verification of the frontend partition boudaries against the backend and frontend and verify they match before proceeding with a partition drop. </li>
...
Buttons:
<ol>
<li>Save : Save any updates to the thresholds, drop action after upload, or drop verify level fields. The button is located in the Offload and Drop Thresholds section.</li>
</ol>
...
<li>Add Row :  Add row to the parition key columns. </li>
...
<li>Save below job step and add parameters for it - Save the job step and go to the parameters page to add parameters to the job step. After the parameters page, you will be returned to Step 2 of the wizard.</li>
...
The currently selected card will have a green border around it. The jobs steps will appear below the job cards section, along withe parameters for the currently selected job step. Additionally, the notifications for the job will be shown last in a collapsable section (click the arrow to collapse or expand the section).
...
<li>Offload Specification : The value can be used for some specialized offloads. Should only be used with help from Conductor for Gluent support.</li>
<li>Parameters : Link to maintain parameters for the job step. </li>
</ol>

Links:
<ol>
<li>Maintain : Pressing the button will bring up the offload parameters page allowing parameters to be added at the job step level. See the Command Parameters section for details on adding parameters.</li>
</ol>
...
<ol>
<li>
Abort the Job Run - Change the state of the job to aborted. The run job cannot be resumed or started once this is done. The job will run from the beginnning on the next scheduled run.
</li>
<li>
...
<li>Database Level - Database level notification will be used when their are no notifications set at the job level.</li>
...
# Appendix A: Jobs - Duplicate Table Job Configuration
